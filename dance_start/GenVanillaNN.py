...
def train(self, n_epochs=20):
    ...
    self.netG.train()  # Met le modèle en mode entraînement
    criterion = nn.MSELoss()  # Définir la fonction de perte (Mean Squared Error)
    optimizer = torch.optim.Adam(self.netG.parameters(), lr=0.0002, betas=(0.5, 0.999))  # Définir l'optimiseur Adam

    for epoch in range(n_epochs):
        for i, data in enumerate(self.dataloader, 0):
            ske, target = data  # Charger les données (squelettes et images cibles)
            optimizer.zero_grad()  # Réinitialiser les gradients

            output = self.netG(ske)  # Passer les squelettes à travers le générateur
            loss = criterion(output, target)  # Calculer la perte entre la sortie et l'image cible
...
def generate(self, ske):
    ...
    #self.netG.eval()  # Met le modèle en mode évaluation
    with torch.no_grad():  # Désactive la calcul des gradients pour accélérer le processus
        ske_t = self.dataset.preprocessSkeleton(ske)  # Prétraiter le squelette
        ske_t_batch = ske_t.unsqueeze(0)  # Ajouter une dimension pour créer un batch
        normalized_output = self.netG(ske_t_batch)  # Passer le squelette à travers le générateur
        res = self.dataset.tensor2image(normalized_output[0])  # Convertir le tenseur en image
        return res  # Retourner l'image générée
